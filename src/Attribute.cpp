...
napi_value Attribute::New(napi_env env, napi_callback_info info) {
	...
	self->env_ = env;
	NAPI_CALL(napi_wrap(env, jsthis, reinterpret_cast<void *>(self), Attribute::Destructor,
...
Attribute::Attribute(const char *name_, int var_id_, int parent_id_)
	: name(name_), var_id(var_id_), parent_id(parent_id_) {}

Attribute::Attribute(const char *name_, int var_id_, int parent_id_, int type_)
...
napi_value Attribute::Init(napi_env env, napi_value exports) {
	...
	napi_value cons;
	NAPI_CALL(napi_define_class(env, "Attribute", NAPI_AUTO_LENGTH, Attribute::New, nullptr, 3,
								properties, &cons));

	NAPI_CALL(napi_set_named_property(env, exports, "Attribute", cons));

	NAPI_CALL(napi_create_reference(env, cons, 0, &Attribute::constructor));
...
napi_value Attribute::GetValue(napi_env env, napi_callback_info info) {
	...
	NAPI_CALL(napi_create_arraybuffer(env, len * type_sizes[self->type], &buffer_data, &buffer));

	NAPI_CALL(napi_create_typedarray(env, type, len, buffer, 0, &array));
...
napi_value Attribute::SetValue(napi_env env, napi_callback_info info) {
	...
	case NC_BYTE: {
		...
		NC_CALL(
			nc_put_att(self->parent_id, self->var_id, self->name.c_str(), self->type, 1, &value));
	} break;
	case NC_SHORT: {
	...
	case NC_STRING: {
		...
		NC_CALL(nc_put_att_text(self->parent_id, self->var_id, self->name.c_str(), strlen(value),
								value));
	} break;
	default:
		napi_throw_error(env, NULL, "Variable type not supported yet");
