...
napi_value Dimension::New(napi_env env, napi_callback_info info) {
	...
	self->env_ = env;
	NAPI_CALL(napi_wrap(env, jsthis, reinterpret_cast<void *>(self), Dimension::Destructor,
...
napi_value Dimension::Init(napi_env env, napi_value exports) {
	...
	napi_value cons;
	NAPI_CALL(napi_define_class(env, "Dimension", NAPI_AUTO_LENGTH, Dimension::New, nullptr, 4,
								properties, &cons));

	NAPI_CALL(napi_set_named_property(env, exports, "Dimension", cons));

	NAPI_CALL(napi_create_reference(env, cons, 0, &Dimension::constructor));
