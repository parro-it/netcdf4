...
napi_value Variable::Init(napi_env env, napi_value exports) {
	napi_property_descriptor properties[] = {
		...
		DECLARE_NAPI_PROP("compressionshuffle", Variable::GetCompressionShuffle,
						  Variable::SetCompressionShuffle),
		DECLARE_NAPI_PROP("compressiondeflate", Variable::GetCompressionDeflate,
						  Variable::SetCompressionDeflate),
		DECLARE_NAPI_PROP("compressionlevel", Variable::GetCompressionLevel,
						  Variable::SetCompressionLevel),
	};

	napi_value cons;

	NAPI_CALL(napi_define_class(env, "Variable", NAPI_AUTO_LENGTH, Variable::New, nullptr, 22,
								properties, &cons));

	NAPI_CALL(napi_set_named_property(env, exports, "Variable", cons));
...
napi_value Variable::New(napi_env env, napi_callback_info info) {
	...
	NAPI_CALL(napi_wrap(env, jsthis, reinterpret_cast<void *>(var), Variable::Destructor,
...
napi_value Variable::Write(napi_env env, napi_callback_info info) {
	...
	for (int i = 0; i < obj->ndims; i++) {
		pos[i] =
	static_cast<size_t>(args[i]->IntegerValue(isolate->GetCurrentContext()).ToChecked()); size[i] =
	1;
	}
	int retval;
	switch (obj->type) {
		...
		case NC_FLOAT: {
			float v =
	static_cast<float>(args[obj->ndims]->NumberValue(isolate->GetCurrentContext()).ToChecked());
			retval = nc_put_vara(obj->parent_id, obj->id, pos, size, &v);
		} break;
		case NC_DOUBLE: {
	...
	"Variable type not supported yet", v8::NewStringType::kNormal).ToLocalChecked())); delete[] pos;
			delete[] size;
			return;
	}
	if (retval != NC_NOERR) {
...
napi_value Variable::WriteSlice(napi_env env, napi_callback_info info) {
	...
	a typed array", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
	size_t* pos = new size_t[obj->ndims];
	size_t* size = new size_t[obj->ndims];
	size_t total_size = 1;
	for (int i = 0; i < obj->ndims; i++) {
		pos[i] = static_cast<size_t>(args[2 *
	i]->IntegerValue(isolate->GetCurrentContext()).ToChecked()); size_t s =
	static_cast<size_t>(args[2 * i + 1]->IntegerValue(isolate->GetCurrentContext()).ToChecked());
		size[i] = s;
		total_size *= s;
	}
	...
	if (val->Length() != total_size) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Wrong
	size of array", v8::NewStringType::kNormal).ToLocalChecked())); delete[] pos; delete[] size;
		return;
	}
	...
	if (!correct_type) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Wrong
	array type", v8::NewStringType::kNormal).ToLocalChecked())); delete[] pos; delete[] size;
		return;
	}
	int retval = nc_put_vara(obj->parent_id, obj->id, pos, size,
	val->Buffer()->GetContents().Data()); if (retval != NC_NOERR) { throw_netcdf_error(isolate,
	retval);
	}
	delete[] pos;
	delete[] size;
...
napi_value Variable::WriteStridedSlice(napi_env env, napi_callback_info info) {
	...
	size_t total_size = 1;
	for (int i = 0; i < obj->ndims; i++) {
		pos[i] = static_cast<size_t>(args[3 *
	i]->IntegerValue(isolate->GetCurrentContext()).ToChecked()); size_t s =
	static_cast<size_t>(args[3 * i + 1]->IntegerValue(isolate->GetCurrentContext()).ToChecked());
		size[i] = s;
		total_size *= s;
		stride[i] = static_cast<ptrdiff_t>(args[3 * i +
	2]->IntegerValue(isolate->GetCurrentContext()).ToChecked());
	}
...
napi_value Variable::Read(napi_env env, napi_callback_info info) {
	...
	if (obj->type < NC_BYTE || obj->type > NC_UINT) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Variable
	type not supported yet", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
	size_t* pos = new size_t[obj->ndims];
	size_t* size = new size_t[obj->ndims];
	for (int i = 0; i < obj->ndims; i++) {
	...
	nc_get_vara(obj->parent_id, obj->id, pos, size, &v); result = v8::Integer::New(isolate, v); }
	break; case NC_INT: { int32_t v; retval = nc_get_vara(obj->parent_id, obj->id, pos, size, &v);
			result = v8::Integer::New(isolate, v);
		} break;
		case NC_FLOAT: {
			float v;
...
napi_value Variable::ReadSlice(napi_env env, napi_callback_info info) {
	...
	NAPI_CALL(
		napi_create_arraybuffer(env, total_size * type_sizes[self->type], &buffer_data, &buffer));

	NC_CALL(nc_get_vara(self->parent_id, self->id, pos, size, buffer_data));

	napi_typedarray_type type = typedarray_cons_per_type(env, self->type);

	napi_value result;

	NAPI_CALL(napi_create_typedarray(env, type, total_size, buffer, 0, &result));
...
napi_value Variable::ReadStridedSlice(napi_env env, napi_callback_info info) {
	...
	NC_CALL(nc_get_vars(self->parent_id, self->id, pos, size, stride, buffer_data));
...
napi_value Variable::SetEndianness(napi_env env, napi_callback_info info) {
	...
	if (arg == "little") {
		...
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Unknown
value", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
...
napi_value Variable::SetChunkSizes(napi_env env, napi_callback_info info) {
	...
	if (array->Get(isolate->GetCurrentContext(), v8::String::NewFromUtf8(isolate, "length",
	v8::NewStringType::kNormal).ToLocalChecked()).ToLocalChecked()->Int32Value(isolate->GetCurrentContext()).ToChecked()
	!= obj->ndims) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Wrong
	array size", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
	int v;
	int retval = nc_inq_var_chunking(obj->parent_id, obj->id, &v, NULL);
	if (retval != NC_NOERR) {
	...
	size_t* sizes = new size_t[obj->ndims];
	for (int i = 0; i < obj->ndims; i++) {
		sizes[i] = array->Get(isolate->GetCurrentContext(),
	i).ToLocalChecked()->Uint32Value(isolate->GetCurrentContext()).ToChecked();
	}
	retval = nc_def_var_chunking(obj->parent_id, obj->id, v, sizes);
	if (retval != NC_NOERR) {
		throw_netcdf_error(isolate, retval);
...
napi_value Variable::SetFillMode(napi_env env, napi_callback_info info) {
	...
	if (!val->IsBoolean()) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Expecting
a boolean", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
...
napi_value Variable::GetFillValue(napi_env env, napi_callback_info info) {
	...
	break; case NC_INT: { int32_t v; retval = nc_inq_var_fill(obj->parent_id, obj->id, NULL, &v);
			result = v8::Integer::New(isolate, v);
		} break;
		case NC_FLOAT: {
			float v;
		...
		default:
			isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate,
	"Variable type not supported yet", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
	if (retval != NC_NOERR) {
		throw_netcdf_error(isolate, retval);
		return;
...
napi_value Variable::SetCompressionLevel(napi_env env, napi_callback_info info) {
	...
	if (!val->IsUint32()) {
		isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "Expecting
	a non-negative integer", v8::NewStringType::kNormal).ToLocalChecked())); return;
	}
